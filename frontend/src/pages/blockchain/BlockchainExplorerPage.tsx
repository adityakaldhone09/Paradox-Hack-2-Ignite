    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="p-4 border-slate-800">
          <span className="text-xs text-slate-400 font-medium">Consortium Network</span>
          <p className="text-sm font-bold text-white mt-1 truncate">{status?.network || 'VeriQ-Proof-Ledger'}</p>
        ...
        <Card className="p-4 border-slate-800">
          <span className="text-xs text-slate-400 font-medium">Mined Block Height</span>
          <p className="text-2xl font-mono font-bold text-brand-400 mt-1">#{status?.block_height ?? 1}</p>
          <span className="text-[11px] text-slate-500">100% Validated</span>
        </Card>

        <Card className="p-4 border-slate-800">
          <span className="text-xs text-slate-400 font-medium">Anchored Transactions</span>
          <p className="text-2xl font-mono font-bold text-indigo-400 mt-1">{status?.total_transactions ?? 28}</p>
          <span className="text-[11px] text-slate-500">Zero Rollbacks</span>
        </Card>

        <Card className="p-4 border-slate-800">
          <span className="text-xs text-slate-400 font-medium">Consensus Mechanism</span>
          <p className="text-xs font-mono font-semibold text-slate-200 mt-1">PoA / SHA-256 Merkle</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {filteredTxs.map((tx) => (
                <tr key={tx.tx_hash} className="hover:bg-slate-900/50 transition-colors">
                  <td className="py-3">
                    <HashViewer hash={tx.tx_hash} truncate={true} prefixLen={8} suffixLen={6} />
                  </td>
                  <td className="py-3 font-mono font-bold text-slate-300">#{tx.block_number}</td>
                  <td className="py-3 font-mono text-brand-400 font-semibold">{tx.event_type}</td>
                  <td className="py-3 text-slate-300 truncate max-w-[150px]">{tx.actor_id}</td>
          ...
          <div className="space-y-4 text-xs">
            <div className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-slate-950 border border-slate-800">
              <div>
                ...
                <span className="font-mono text-brand-400 font-bold">{selectedTx.event_type}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Block Number:</span>
                <span className="font-mono text-slate-200 font-bold">#{selectedTx.block_number}</span>
              </div>
              <div>
                <span className="text-slate-500 block">Actor:</span>
                <span className="text-slate-200 font-medium">{selectedTx.actor_id}</span>
              </div>
              <div>
            ...
            <div>
              <span className="text-slate-400 block mb-1 font-semibold">Transaction ID (Hash):</span>
              <HashViewer hash={selectedTx.tx_hash} truncate={false} />
            </div>

            <div>
              <span className="text-slate-400 block mb-1 font-semibold">Payload Merkle / Content Hash:</span>
              <HashViewer hash={selectedTx.payload_hash} truncate={false} />
            </div>

            <div>
              <span className="text-slate-400 block mb-1 font-semibold">Previous Block Cryptographic Link:</span>
              <HashViewer hash={selectedTx.previous_hash} truncate={false} />
            </div>

            <div>
              <span className="text-slate-400 block mb-1 font-semibold">Digital Authority Signature:</span>
