    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 bg-slate-900/60 p-3 rounded-xl border border-slate-800 text-xs">
        ...
        <select
          ...
          className="bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-slate-200 focus:outline-none focus:border-brand-500"
        >
              ...
              {incidents.length === 0 ? (
                ...
                incidents.map((inc) => (
                  <tr key={inc.id} className="hover:bg-slate-900/50 transition-colors">
                    <td className="py-3 font-mono text-brand-400 font-bold">{inc.incident_id}</td>
                    <td className="py-3">
                      <StatusBadge status={inc.severity} />
                    </td>
                    <td className="py-3 font-mono text-slate-300 font-medium">{inc.type}</td>
                    <td className="py-3 text-slate-200">{inc.centre_name || 'Central Platform'}</td>
                    ...
                    <td className="py-3 text-right space-x-1.5">
                      {inc.status === 'OPEN' && (
                        <button
                          ...
                          className="px-2 py-1 rounded bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 font-medium text-[11px]"
                        >
                          Acknowledge
                        </button>
                      )}
                      {inc.status !== 'RESOLVED' && (
                        <button
                          ...
                          className="px-2 py-1 rounded bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 font-medium text-[11px]"
                        >
                          Resolve
                        </button>
                      )}
      ...
      <Modal isOpen={isResolveOpen} onClose={() => setIsResolveOpen(false)} title="Resolve Security Incident">
        <form onSubmit={handleResolve} className="space-y-4 text-xs">
          <p className="text-slate-300">
            Resolving Incident <span className="font-mono text-brand-400 font-bold">{selectedIncident?.incident_id}</span>
          </p>
          <div>
            <label className="block font-semibold text-slate-300 mb-1">Resolution Investigation Notes *</label>
            <textarea
              required
              rows={3}
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-slate-200 focus:outline-none focus:border-brand-500"
            />
          </div>
