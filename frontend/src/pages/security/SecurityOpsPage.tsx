...
export const SecurityOpsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-5 border-slate-800 bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950/30">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-9 h-9 rounded-xl bg-purple-500/10 border border-purple-500/30 flex items-center justify-center text-purple-400">
            <Cpu className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-base font-bold text-white">AI Behavioral Anomaly Detection Engine</h3>
            <span className="text-[11px] text-purple-400 font-medium">Deterministic Heuristic Baseline Model</span>
          </div>
        </div>
        ...
        <p className="text-xs text-slate-300 leading-relaxed">
          ...
          <strong className="text-brand-400">Risk indications</strong> rather than definitive proof of malicious behaviour.
        </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-3 border-t border-slate-800 text-xs">
          ...
          <div>
            ...
            <span className="font-mono text-slate-200">5 req / 60s max</span>
          </div>
          <div>
            <span className="text-slate-500 block">Hardware Validation:</span>
            <span className="font-mono text-slate-200">SHA-256 TPM Fingerprint</span>
          </div>
          <div>
            <span className="text-slate-500 block">Evaluation Protocol:</span>
            <span className="text-brand-400 font-mono">Consensus-Linked</span>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-5 border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">Aggregated incidents & blocked access scores</span>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="font-mono text-xs font-bold text-slate-100">{item.centre_id}</span>
                  <StatusBadge status={item.risk_level} showIcon={false} />
                </div>
                <h4 className="text-xs font-semibold text-white truncate">{item.centre_name}</h4>
                <p className="text-[10px] text-slate-400 truncate">{item.city}</p>

                <div className="flex justify-between items-center text-[11px] pt-2 mt-2 border-t border-slate-800/80">
                  <span className="text-slate-500">Risk Index:</span>
                  <span className="font-mono font-bold">{item.risk_score}/100</span>
                </div>
              </div>
            );
          })}
      ...
      <Card className="p-5 border-slate-800 space-y-3">
        ...
        <div className="space-y-2.5">
          {threatFeed.map((item) => (
            <div key={item.id} className="p-3 rounded-lg bg-slate-950 border border-slate-800/80 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <StatusBadge status={item.severity} />
                  <span className="font-mono text-slate-300 font-bold">{item.type}</span>
